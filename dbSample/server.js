...
app.post('/student/update', async (req, res) => {
    ...
    // Check if course is a valid number
    if (isNaN(updatedStudentData.course)) {
        return res.status(400).send('Invalid course value');
    }

    try {
        await collegeData.updateStudent(updatedStudentData);
